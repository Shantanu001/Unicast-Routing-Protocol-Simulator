...
public class BGP_protocol extends JFrame {
	...
	String row[][] = { { "1", "" }, { "2", "" }, { "3", "" }, { "4", "" }, { "5", "" }, { "6", "" } }; // routing table
																										// values
	...
	String dups_row[][] = { { "1", "" }, { "2", "" }, { "3", "" }, { "4", "" }, { "5", "" }, { "6", "" } }; // duplicate
																											// table to
																											// initialise
																											// the
																											// vlaues
	...
	BGP_protocol() {
		...
		next_submit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				for (int i = 1; i <= temp_node_per_as; i++) {
					delete_selected_node_per_as(temp_value);
					temp_value--;
				}
		...
		n_num.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent arg0) {
				...
				num_nodes = (int) n_num.getValue();
				for (int i = 1; i <= node_size; i++) {
					node_list[i] = i;
					node_selector_list[i] = i;
					node_per_as_list[i] = i;
				}
			}
		});
		...
		submit.addActionListener(new ActionListener() { // function for reset button
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 1; i <= (int) as_num.getValue(); i++) {
					if (speaker_node[i] != 999) {
						...
						if (speaker_node[i] == 1) {
							...
							for (int j = 0; j < (int) n_num.getValue(); j++) {
				...
				for(int qq=(int) n_num.getValue();qq<6;qq++)
				{
					row[qq][0]="";
					row1[qq][0]="";
					row2[qq][0]="";
